...
- [x] **阶段 4**：拍题作答（框选题目 → OCR → 大模型解答 → 自动填充，热键 Ctrl+Alt+X）
- [x] **阶段 5**：输入框实时翻译（选中文本 → 热键 Ctrl+Alt+T → 原地替换/弹窗）
- [x] **阶段 6**：浏览器插件 + 本地服务（FastAPI 服务器 + Chrome/Edge 扩展）
...
- 主窗口「拍题作答」按钮、托盘菜单，或全局热键 **Ctrl+Alt+X**
- 框选题目区域 → OCR 识别 → 大模型解答 → 弹窗显示答案与解析
- 弹窗支持「复制答案」与「自动填充」（点击后切到目标输入框，自动键入答案）
- 答案语言跟随界面语言设置

## 输入框实时翻译（阶段 5）

- 在任意输入框选中文本 → 按全局热键 **Ctrl+Alt+T**（或主窗口「输入框翻译」按钮）
- 通过剪贴板复制选区 → 翻译 → **原地替换**选中文本（默认），完成后恢复原剪贴板
- 可在设置里关闭「直接替换」，改为**弹窗**显示译文
- 基于剪贴板方案，兼容几乎所有 Windows 输入框，无需逐应用适配

## 浏览器插件 + 本地服务（阶段 6）

- 应用启动时自动运行 **FastAPI 服务器**（`127.0.0.1:18190`），可在托盘菜单切换
- 浏览器扩展位于 `browser-extension/` 目录
- **安装方式**：Chrome/Edge 打开 `chrome://extensions` → 开启开发者模式 → 「加载已解压的扩展」 → 选择 `browser-extension/` 文件夹
- **功能**：
  - 右键选中文字 → 「Translate with AutoTranslate」 → 弹窗显示译文
  - 扩展图标弹窗：输入文字即时翻译
  - 网页内选中文字 → 浮动「T」按钮 → 点击翻译
